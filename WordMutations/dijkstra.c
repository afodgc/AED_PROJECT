 ...
 * 
 * g : graph
 * origem : index do vertice de origem
 * destino : index do vertice de destino 
 * wt : custos
 * st : antecessores
 * numOfMutations : (limite do algoritmo) numero de mutações
 * 
 * side efects: encontra o caminho mais entre dois vertices
***********************************************************************/
void dijkstra(Graph *G, int origem, int destino, int *st, float *wt, int numOfMutations)
{
    Queu Queu; // fila
    int custo_max = numOfMutations * numOfMutations; // distancia máxiam atá áà origem
